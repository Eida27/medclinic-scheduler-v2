...
import { authorizeAuthenticatedStaff, authorizeSession } from "@/server/services/auth.service";
...
export async function requireAuthenticatedStaff(allowedRoles?: UserRole[]) {
  const token = (await cookies()).get(SESSION_COOKIE)?.value;
  if (!token) throw new AppError("UNAUTHENTICATED", "Please sign in to continue.", 401);
  try {
    return await authorizeAuthenticatedStaff(await verifySessionToken(token), allowedRoles);
  } catch (error) {
    if (error instanceof AppError) throw error;
    throw new AppError("UNAUTHENTICATED", "Please sign in to continue.", 401);
  }
