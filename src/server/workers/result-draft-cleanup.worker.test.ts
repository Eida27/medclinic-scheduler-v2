...
describe("result draft cleanup", () => {
  ...
  it("retains active and finalized submissions", async () => {
    ...
    await finalizeStudentResultSubmission("99-9303-03", finalized.appointmentId);
    const now = new Date("2027-09-08T00:00:00.000Z");
    await pool.query(
  ...
  it("leaves failed deletions retryable and succeeds on a later idempotent pass", async () => {
    ...
    const failingStorage = { ...storage, delete: async () => { throw new Error("synthetic cleanup failure"); } };
  ...
  it("retries physical deletion markers left by invalidation", async () => {
    ...
    const finalized = await finalizeStudentResultSubmission("99-9305-05", fixture.appointmentId);
    ...
    await cleanupExpiredResultDrafts(new Date("2027-09-08T00:00:00.000Z"), storage);
