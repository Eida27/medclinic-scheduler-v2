...
export async function complianceReport(filters: {
  ...
  const joins = `
    ...
    LEFT JOIN LATERAL (SELECT status FROM appointments WHERE student_number=s.student_number AND status NOT IN ('RESCHEDULED','CANCELLED') ORDER BY appointment_date DESC, created_at DESC LIMIT 1) latest_appointment ON TRUE
...
export async function dashboardMetrics() {
  const result = await query<{
    ...
    SELECT
      ...
      (SELECT COUNT(*)::int FROM exam_results WHERE result_status='COMPLETED') AS completed_exam,
      (SELECT COUNT(*)::int FROM laboratory_results WHERE result_status='COMPLETED') AS completed_lab,
      (SELECT COUNT(*)::int FROM appointments WHERE status='NO_SHOW') AS no_shows,
      (SELECT COUNT(*)::int FROM appointments WHERE status='RESCHEDULED') AS rescheduled,
      (SELECT COUNT(*)::int FROM schedule_batches WHERE status IN ('DRAFT','VALIDATED','GENERATED')) AS unpublished_batches,
      (SELECT COUNT(*)::int FROM (
        SELECT a.appointment_date,a.schedule_type FROM appointments a
        JOIN clinic_capacity_settings c ON c.schedule_type=a.schedule_type
        WHERE a.status IN ('DRAFT','PENDING') GROUP BY a.appointment_date,a.schedule_type,c.safe_daily_capacity
