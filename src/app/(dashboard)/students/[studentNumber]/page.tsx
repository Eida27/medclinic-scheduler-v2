...
export default async function StudentDetailsPage({ params }: { params: Promise<{ studentNumber: string }> }) {
  ...
  return (
    <>
      ...
      <Card><CardTitle>Appointment history</CardTitle><div className="mt-4 grid gap-3">{student.appointments.map((appointment: AppointmentHistory) => <div key={appointment.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-50 p-4"><div><p className="font-bold">{appointment.schedule_type.replaceAll("_", " ")}</p><p className="text-sm text-slate-600">{appointment.appointment_date}</p></div><Badge tone={appointment.status === "COMPLETED" ? "success" : appointment.status === "NO_SHOW" ? "danger" : "neutral"}>{appointment.status}</Badge></div>)}{student.appointments.length === 0 ? <p className="text-sm text-slate-500">No appointments yet.</p> : null}</div></Card>
    </>
  );
}
