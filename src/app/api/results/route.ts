...
export async function POST(request: Request) { try { const user = await requireUser(); return dataResponse(await recordResult(await request.json(), user.userId), { status: 201 }); } catch (error) { return errorResponse(error); } }
