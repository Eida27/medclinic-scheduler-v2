...
export async function DELETE(request: Request) {
  try { const user = await requireUser(["ADMIN"]); return dataResponse(await removeReference("program", await request.json(), user.userId)); } catch (error) { return errorResponse(error); }
}
